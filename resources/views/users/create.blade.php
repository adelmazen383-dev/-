<x-app-layout>
    ...
    <div class="card max-w-2xl mx-auto">
        <form method="POST" action="{{ route('users.store') }}" class="space-y-6">
            ...
            <div>
                <label class="block text-sm font-medium text-slate-300 mb-2">الاسم الرباعي</label>
                <input type="text" name="name" value="{{ old('name') }}" class="form-input w-full" required autofocus>
                @error('name') <p class="text-rose-400 text-xs mt-1">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="block text-sm font-medium text-slate-300 mb-2">البريد الإلكتروني (لتسجيل الدخول)</label>
            ...
            <div>
                <label class="block text-sm font-medium text-slate-300 mb-2">الصلاحية</label>
